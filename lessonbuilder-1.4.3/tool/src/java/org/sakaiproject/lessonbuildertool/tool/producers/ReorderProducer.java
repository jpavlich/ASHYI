...
public class ReorderProducer implements ViewComponentProducer, NavigationCaseReporter, ViewParamsReporter {
	...
	public void fillComponents(UIContainer tofill, ViewParameters params, ComponentChecker checker) {
		...
		if (simplePageBean.canEditPage()) {
			...
			List<Actividad> actividades = simplePageBean.getActividadesEnUnidad(currentPage.getTitle());
			...
			for (SimplePageItem i : items) {
				...
				if (i.getType() == 5) {
					...
					{
						showPageProducer.makeLink(row, "link", i, simplePageBean, simplePageToolDao, messageLocator, true, currentPage, false, Status.NOT_REQUIRED, actividades.get(nA), new ItemsUsuarioImpl(), (GeneralViewParameters)params);
						nA++;
					}
					else
					{
						...
						a.setNombre("");
						showPageProducer.makeLink(row, "link", i, simplePageBean, simplePageToolDao, messageLocator, true, currentPage, false, Status.NOT_REQUIRED, a, new ItemsUsuarioImpl(),(GeneralViewParameters)params);
					}
				}
			}
